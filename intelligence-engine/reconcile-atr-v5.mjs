...
// Fetch raw metadata alongside sessions
async function fetchAllAcuitySessions(assetId) {
  const endDate   = new Date().toISOString().slice(0,10)
  const startDate = new Date(Date.now() - 180*24*60*60*1000).toISOString().slice(0,10)
  const body = new URLSearchParams({ startDate, endDate, assetId: String(assetId), interval: '0' })
  const res = await fetch(`${ACUITY_PRICE_URL}?apikey=${ACUITY_API_KEY}`, {
    method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: body.toString()
  })
  const data = await res.json()
  const rows = data.filter(d => d.p)
  const sessionMap = new Map()
  for (const row of rows) {
    const shifted = new Date(new Date(row.date).getTime() + 2*60*60*1000)
    const fxDate = shifted.toISOString().slice(0,10)
    if (!sessionMap.has(fxDate)) sessionMap.set(fxDate, 0)
    sessionMap.set(fxDate, sessionMap.get(fxDate) + 1)
  }
  return {
    rawCount: data.length,
    earliest: data[0]?.date ?? 'n/a',
    latest: data[data.length-1]?.date ?? 'n/a',
...
async function fetchDailySessions(symbol) {
  ...
  // Reverse so chronological order for ATR calculation
...
for (const mkt of MARKETS) {
  ...
  try {
    ...
    // Count discarded sessions
    const allAcuitySess = await fetchAllAcuitySessions(mkt.assetId)
    const discarded = allAcuitySess.totalSessions - acuitySessions.length
    const firstAtrDate = acuitySessions.length >= 20 ? acuitySessions[19].date : 'n/a'
    console.log(`Raw hourly rows returned:      ${allAcuitySess.rawCount}`)
    console.log(`Earliest timestamp:            ${allAcuitySess.earliest}`)
    console.log(`Latest timestamp:              ${allAcuitySess.latest}`)
    console.log(`Complete sessions (≥20 hrs):   ${acuitySessions.length}`)
    console.log(`Sessions discarded (<20 hrs):  ${discarded}`)
    console.log(`First non-null ATR20 date:     ${firstAtrDate}`)
    ...
    // Debug: show sample dates from each source
    const acuityDates = [...acuityByDate.keys()].sort()
    const dailyDates  = [...dailyByDate.keys()].sort()
    console.log(`  Acuity date range: ${acuityDates[0]} → ${acuityDates[acuityDates.length-1]}`)
    console.log(`  Daily date range:  ${dailyDates[0]} → ${dailyDates[dailyDates.length-1]}`)
    console.log(`  Sample Acuity (last 5): ${acuityDates.slice(-5).join(', ')}`)
    console.log(`  Sample Daily  (last 5): ${dailyDates.slice(-5).join(', ')}`)
    ...
    for (const date of commonDates) {
      ...
      // Count how many complete Acuity sessions existed at this point (warm-up depth)
      const warmupSessions = acuitySessions.filter(s => s.date <= date).length
      results.push({
        date, atrAcuity: a.atr, atrDaily: d.atr,
        absDiff, pctDiff,
        upperDiff: Math.abs(upperA - upperD),
        lowerDiff: Math.abs(lowerA - lowerD),
        zoneA, zoneD, zoneChanged, warmupSessions
      })
    ...
    const maxErr = pcts[pcts.length-1]
    ...
    console.log(`  Maximum ATR % error:         ${maxErr.toFixed(2)}%`)
    ...
    // Warm-up bucket analysis
    const buckets = [
      { label: '20-39 sessions', min: 20, max: 39 },
      { label: '40-59 sessions', min: 40, max: 59 },
      { label: '60-79 sessions', min: 60, max: 79 },
      { label: '80-99 sessions', min: 80, max: 99 },
      { label: '100+ sessions',  min: 100, max: Infinity },
    ]
    console.log(`\n  Warm-up bucket analysis:`)
    for (const bucket of buckets) {
      const inBucket = results.filter(r => r.warmupSessions >= bucket.min && r.warmupSessions <= bucket.max)
      if (inBucket.length === 0) continue
      const bPcts = inBucket.map(r => r.pctDiff).sort((a,b) => a-b)
      const bMed  = bPcts[Math.floor(bPcts.length/2)]
      const bMax  = bPcts[bPcts.length-1]
      const bZone = inBucket.filter(r => r.zoneChanged).length
      console.log(`    ${bucket.label.padEnd(18)}: n=${String(inBucket.length).padStart(3)}  median=${bMed.toFixed(2)}%  max=${bMax.toFixed(2)}%  zone_changes=${bZone}/${inBucket.length}`)
    }

    // Approval criteria
    const CRITERIA = { median: 1.0, p90: 2.0, max: 3.0, zonePct: 5.0 }
    const zonePct = zoneChanges / results.length * 100
    const pass = median < CRITERIA.median && p90 < CRITERIA.p90 && maxErr < CRITERIA.max && zonePct < CRITERIA.zonePct
    const verdict = pass
      ? 'PASS — Acuity approved as canonical APIP zone data source'
      : 'FAIL — does not meet approval criteria'
    console.log(`\n  Approval criteria (median<1%, p90<2%, max<3%, zone_diff<5%):`)
    console.log(`    median ${median.toFixed(2)}% ${median < CRITERIA.median ? '✓' : '✗'}  p90 ${p90.toFixed(2)}% ${p90 < CRITERIA.p90 ? '✓' : '✗'}  max ${maxErr.toFixed(2)}% ${maxErr < CRITERIA.max ? '✓' : '✗'}  zone ${zonePct.toFixed(1)}% ${zonePct < CRITERIA.zonePct ? '✓' : '✗'}`)
