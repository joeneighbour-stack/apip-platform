...
// Matches ZoneLadder.tsx's total height (268px) so the two panels sit at the
// same height as a visual pair -- this chart scales its own y-axis to its own
// price range (zones + entry/stop/target), independently of the ladder; the
// two are no longer trying to share a price-to-pixel mapping.
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 0, right: 8, bottom: 0, left: 0 }}>
          <XAxis
            ...
            tickLine={false}
          />
          {/* Visible again -- the chart is its own price-history panel now, not
              a seamless extension of the ladder, so it needs to be readable on
              its own. domain still comes from zoneBoundaries/entry range so the
              scale makes sense; it just isn't trying to pixel-match the ladder. */}
          <YAxis
            domain={domain}
            tick={{ fontSize: 10 }}
