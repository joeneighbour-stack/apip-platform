...
export default async function AnalystWorkspacePage() {
  ...
  // Fetch today's coaching recommendations for this analyst,
  // joining through to the opportunity and recommendation_version
  // to get validity status and condition warnings.
  const { data: recommendations } = await supabase
    ...
    .select(`
      ...
      markets:opportunities!inner ( markets!inner ( symbol, asset_class ) ),
      recommendation_versions!active_recommendation_version_id (
  ...
  // Fetch post-trade reviews pending acknowledgement
      ...
      <section className="space-y-3">
        ...
        {!recommendations || recommendations.length === 0 ? (
          ...
          recommendations.map((rec) => {
            const opp = rec.opportunities as any
            const rv = rec.recommendation_versions as any
            const market = (rec.markets as any)?.markets
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground shrink-0">
                    <span>
                      Trigger <span className="font-medium text-foreground">
                        {Math.round((rec.trigger_probability ?? 0) * 100)}%
                      </span>
                    </span>
                    <span>
                      Expected R <span className="font-medium text-foreground">
                        {Number(rec.expected_r).toFixed(2)}R
                      </span>
                    </span>
                  </div>
                </div>
        ...
        {!reviews || reviews.length === 0 ? (
          ...
          reviews.map((review) => (
            <div key={review.review_id}
              ...
              <AcknowledgeButton reviewId={review.review_id} />
            </div>
          ))
        )}
      </section>
...
// Client component for the acknowledge action
function AcknowledgeButton({ reviewId }: { reviewId: string }) {
  'use client'
  // Placeholder -- will wire to a server action in the next step
  return (
    <button
      className="text-xs px-3 py-1.5 rounded-md border border-border hover:bg-muted transition-colors text-muted-foreground"
      onClick={() => alert(`Acknowledging review ${reviewId} -- server action coming next`)}
    >
      Acknowledge
    </button>
  )
