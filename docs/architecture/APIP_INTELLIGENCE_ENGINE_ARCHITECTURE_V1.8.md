# APIP Intelligence Engine Architecture V1.8 — FROZEN BASELINE
...
**Supersedes:** V1.7. Widens `ShadowTradeService.templateSource` to its full intended vocabulary, and names `ShadowOutcomeLifecycleService` as an explicit, separately-designed future service rather than leaving outcome evolution as an undifferentiated gap.
...
This is the approved, frozen baseline (V1.8) — the seven resolutions in Section 1, the V1.1-V1.8 amendments (see Changelogs above), and the Section 3.9 schema restructuring have already been reviewed and approved. Section 1 is still the most important section to read first, since every other section depends on its resolutions, but it is no longer a set of open recommendations awaiting sign-off — it is the contract. Any future disagreement with a resolution here is a proposed amendment against this baseline (a new versioned document, V1.9 and onward), not an in-place edit.
...
*End of APIP Intelligence Engine Architecture V1.8 — frozen baseline. This document, not the notebook directly, is the contract implementation must satisfy. Future architectural changes are proposed as versioned amendments against this baseline (V1.9, V1.10, ...), never as direct edits to this file.*
