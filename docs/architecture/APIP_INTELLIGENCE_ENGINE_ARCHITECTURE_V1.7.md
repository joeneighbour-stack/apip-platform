# APIP Intelligence Engine Architecture V1.7 — FROZEN BASELINE
...
**Supersedes:** V1.6. Completes Phase 1.5 Step 7: `ShadowTradeService` creation logic, corrected against the real live `shadow_trades`/`shadow_trade_outcomes` schema (verified directly via `information_schema.columns` before writing any code) rather than assumed from the notebook's flat output shape — the same discipline that caught Step 5's `RecommendationService` mismatch.
...
This is the approved, frozen baseline (V1.7) — the seven resolutions in Section 1, the V1.1-V1.7 amendments (see Changelogs above), and the Section 3.9 schema restructuring have already been reviewed and approved. Section 1 is still the most important section to read first, since every other section depends on its resolutions, but it is no longer a set of open recommendations awaiting sign-off — it is the contract. Any future disagreement with a resolution here is a proposed amendment against this baseline (a new versioned document, V1.8 and onward), not an in-place edit.
...
```typescript
type TemplateSource = 'historical_template' | 'fallback';
...
*End of APIP Intelligence Engine Architecture V1.7 — frozen baseline. This document, not the notebook directly, is the contract implementation must satisfy. Future architectural changes are proposed as versioned amendments against this baseline (V1.8, V1.9, ...), never as direct edits to this file.*
