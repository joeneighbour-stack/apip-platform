# APIP Intelligence Engine Architecture V1.11 — FROZEN BASELINE
...
**Supersedes:** V1.10. Completes Phase 1.6 (Coaching Layer): `CoachingService` (`buildCoachingRecommendation`, `generateCoachingNote`, `lintAnalystText`) and `ReviewService` (`buildPostTradeReviews`), both corrected against the real live `coaching_recommendations` schema and confirmed `post_trade_reviews` absence before writing any code.

**Scope:** Phase 1.5–1.6 (Intelligence Engine + Coaching Layer). Dashboard layer (1.7) is out of scope here.
**Inputs to this document:** `APIP_NEXTJS_PLATFORM_SPEC_V1_6.xlsx`, `APIP_RESEARCH_ENGINE_V1_0.ipynb`, and the as-built schema (migrations 001–033).

## Changelog: V1.10 → V1.11

### Phase 1.6 built: CoachingService + ReviewService (Sections 3.13/3.14)

**Schema-first check caught a significant mismatch in the initial `CoachingOutput` type**, requiring a full rebuild before any code shipped — same discipline as Steps 5 and 7:

| Field | What I initially built | What the real schema has |
|---|---|---|
| `coachingRecommendationId` | Self-generated UUID | `recommendation_id` (the PK — same concept, different name) |
| `analystId` | String name | UUID, FK → `analysts` |
| `suggestedEntryRange` | Combined text `"1.0875 - 1.09"` | `entry_range_low` + `entry_range_high` as **separate numerics** |
| `suggestedRiskRange` | `"Around 1.0825"` | `risk_range` text — the ATR-band format from `RecommendationService`, passed through |
| `suggestedTargetRange` | `"Towards 1.0975"` | `target_range` text — same |
| `session`, `direction`, `current_zone` etc. | Persisted | Absent — recovered via joins |

The unique constraint on `(opportunity_id, analyst_id)` was also a real finding: one coaching row per analyst per opportunity, not one per recommendation version.

**`post_trade_reviews` confirmed absent** from the live schema — migration 033 creates it, shaped exactly to match `ReviewService`'s `PostTradeReview` output.

**`lint_analyst_text()` and `FORBIDDEN_ANALYST_TERMS`:** six terms confirmed from `PARAMETERS['forbidden_analyst_terms']` in notebook cell 2 — `'shadow trade'`, `'model beat you'`, `'non-compliant'`, `'wrong'`, `'failure'`, `'low confidence'`. These are coaching philosophy guardrails, not profanity filters — the generated text must never imply the model is judging or has outperformed the analyst. Do not add to this list without updating `PARAMETERS` in the notebook first; the notebook is the source of truth.

**`:.5g` format verified byte-for-byte against Python** before use — `parseFloat(value.toPrecision(5)).toString()` matches Python's `:.5g` output exactly for all representative values tested, including edge cases (`100.0` → `"100"`, not `"100.0"`). Same verification discipline as `stableHash()`.

**`risk_range`/`target_range` in `coaching_recommendations` are the ATR-band text strings already computed by `RecommendationService`** (`"1.0815–1.0835"` format) — passed through by `CoachingService`, not recomputed in a new format. The notebook's `"Around X"` / `"Towards Y"` text appears only inside `coaching_note` as embedded guidance, not as separate persisted fields.

**Caller responsibility confirmed:** `buildPostTradeReviews` requires the caller to exclude `historical_backfill === true` trades before calling. Per notebook cell 0: "Historical backfill rows cannot be used for coaching-alignment reviews." The service itself has no I/O so cannot enforce this — documented as a contract invariant.

No other section is affected. All prior amendments remain in force exactly as stated.

---

## Changelog: V1.9 → V1.10

### `ValidationService` built (Section 10) — Phase 1.5 pure-logic services complete

**No notebook formula exists for this service.** The notebook treats equivalence as a principle (cell 14: "The implementation can differ. The behaviour should not.") rather than implementing it. The entire service is designed against Section 10's own contract.

**Migration 032** (`engine_validation_runs` table) applied to staging, confirmed before writing any service code. Includes `resolution_type` column per Section 10.6 (Amendment 4) — distinguishes methodology changes from production bug fixes in the audit trail, as a real persisted column rather than a convention.

**One real bug caught during implementation:** Section 10.5 says `recommendationValidityStatus` is CRITICAL "in a way that would change whether an analyst sees `DO_NOT_USE_RECALCULATE` vs anything else." A naive implementation classified ALL `recommendationValidityStatus` mismatches as CRITICAL — but `VALID↔STALE_PRICE` does not involve `DO_NOT_USE_RECALCULATE` and should be `MEDIUM` (adjacent states at a near-boundary ATR move). Fixed correctly against the exact architecture document definition, not the naive reading. The corrected rule:
- `recommendationValidityStatus` → CRITICAL only if either side is `DO_NOT_USE_RECALCULATE`; `VALID↔STALE_PRICE` → MEDIUM; other mismatches → HIGH
- `direction` / `analystAction` → always CRITICAL
- All other categorical fields → HIGH by default

**One real test bug caught during test-writing:** the LOW-severity tolerance test used `1e-12` as its "small but detectable" delta, which is actually *below* the `1e-9` match threshold — correctly producing no difference entry at all, not a LOW-severity one. Fixed against the actual verified tolerance band rather than hand-calculation.

**Two design decisions of note:**
1. `UNCLASSIFIED` is a genuine, required category — not a bucket that quietly becomes `LOGIC_DIVERGENCE` by default. A human reviewer must classify any `UNCLASSIFIED` drift before it can be resolved.
2. The `differences` array contains only fields that actually differ — not all compared fields every time. This is essential for the report to remain readable as drift surface area grows across the full field set.
...
### 1. `AllocationService` built (Section 3.12) — correcting V1.1 Section 1.7's multi-column scoring claim

**Schema correction:** V1.1 Section 1.7 described five separate scoring columns (`market_fit_score`, `regime_fit_score`, `workload_score`, `availability_score`, `final_score`). Confirmed against the live `coverage_allocation` schema before writing any code: **only `allocation_score` exists**. The notebook itself only ever computes one combined score — the five-column description was an imagined future extension, not a current or planned schema reality. Built here against the real single-column shape.

**Algorithm — stateful and sequential (not parallelisable):**
1. Sort all opportunities by `expected_r` descending
2. For each opportunity in that order, score every eligible analyst: `score = expected_r + (0.2 if analyst === preferredAnalyst else 0) - workload[analyst] * 0.1`
3. Assign the highest-scoring analyst, then **increment their workload before scoring the next opportunity**

Step 3's ordering is critical — an analyst assigned to a high-`expected_r` opportunity has higher workload when the next opportunity is scored, which can tip close decisions toward a less-loaded analyst. Do not parallelize this loop or change the sort order without a research re-validation.

**`reason_summary`:** a literal fixed string from the notebook — "Assigned using expected R, profile fit and workload balancing." — not computed per-allocation, not parameterised. Persisted to `coverage_allocation.reason_summary` (confirmed real column, verified before writing code).

**`recommendation_version_id`:** confirmed as a real column on `coverage_allocation` before finalising the output type, same schema-first discipline as Steps 5 and 7.

**Fallback:** if `eligibleAnalysts` is empty for a given opportunity, falls back to all active analysts (matching the notebook's `list(workload.keys())` fallback). If no analysts exist at all, the opportunity is skipped rather than crashing — an edge case the notebook doesn't handle explicitly, added here for production safety.

**Analyst availability:** pure pass-through — this service receives the already-filtered `eligibleAnalysts` list from its caller and does not query `analyst_availability` directly (consistent with every other service in this engine: pure function, no I/O). The opt-out semantics (no row = available by default) are the caller's responsibility.
...
### 1. `ShadowTradeService.templateSource` widened to its full intended vocabulary (Section 3.11)

V1.7's contract limited `templateSource` to `'historical_template' | 'fallback'` — `TemplateService`'s own classification only. Per product clarification, the real intended vocabulary is five values: `'historical_template' | 'fallback' | 'exact_profile' | 'market_profile' | 'unknown'`, spanning both `TemplateService`'s classification and `AnalystProfileService`'s (`profileSource`), plus an explicit `'unknown'` for when no selection-path diagnostic is available at all.

**`ShadowTradeService` itself does not decide which of the five values applies.** It remains a pure pass-through, persisting whatever value its caller supplies — the decision of whether a given recommendation's shadow trade should be tagged by its template-path or profile-path source belongs to whichever code eventually wires `RecommendationService`'s diagnostics into this service (not yet built; see Section 12 item 4, new below). `'unknown'` is an explicit, honest value a caller supplies when no diagnostic is available — never a silent default this service invents on its own.

**Purpose, stated explicitly per the clarification:** this field exists to support later analysis — did shadow trades perform better when generated from strong historical templates vs. fallbacks? Did `exact_profile`-sourced recommendations outperform `market_profile`-sourced ones? Which template/profile pathways are most automation-ready? Which markets/sessions are reliable enough for systematic recommendation generation? **This is internal-only analysis** — `template_source`, like every `shadow_trades` field, is never exposed to analysts, per the Hidden Boundary (Section 9).

### 2. `confidenceLabel` — no change, decision reaffirmed

Confirmed: literal `null` remains correct for V1, with no formula invented. The clarification additionally named the real factors a future research-validated formula should eventually draw on — template sample size, template average R, trigger probability sample size, profile sample size, event risk, regime stability, shadow-vs-actual performance — recorded here as a pointer for whoever eventually does that research, not as a formula to implement now.

### 3. `ShadowOutcomeLifecycleService` named as an explicit future service, designed separately from `ShadowTradeService`

Per product clarification, outcome evolution (Section 12 item 3) should not remain an undifferentiated gap — it has a name and a clear boundary now, even though it is not built:

- **Purpose:** track whether a shadow trade becomes `NOT_TRIGGERED` → `TRIGGERED` → `TARGET_HIT` / `STOP_HIT` / `EXPIRY` / `CANCELLED` / `AMBIGUOUS` (the full `shadow_trade_outcomes.trade_outcome_status` enum, already live in the schema).
- **Inputs:** the shadow trade's immutable `entry`/`stop`/`target` (from `ShadowTradeService`), an expiry rule, intraday market data, and session rules.
- **Explicit separation of concerns:** `ShadowTradeService` creates the immutable benchmark setup and must never be extended to also monitor or resolve outcomes — that is `ShadowOutcomeLifecycleService`'s job entirely, designed and built separately, on its own schedule, against its own (not yet written) contract.

This is a naming and boundary decision, not an implementation — `ShadowOutcomeLifecycleService` has zero notebook precedent (confirmed in Step 7's work) and needs its own design pass before any code is written.

No other section is affected. All prior amendments remain in force exactly as stated in their respective changelogs.

---

## Changelog: V1.6 → V1.7

### `ShadowTradeService` contract corrected against the real schema (Section 3.11)

The notebook's `build_shadow_trades` copies `market`/`session`/`direction`/`trigger_probability`/`expected_r`/`parameter_snapshot_hash`/`shadow_status` directly onto the shadow trade record. **None of these are real columns on `shadow_trades`** — confirmed by querying `information_schema.columns` before writing any code, not assumed either way. This validates Section 1.6's existing resolution (these are recoverable via the `recommendation_version_id`/`opportunity_id` FKs and a join, not duplicated) — the resolution was already correct, it just hadn't been checked against the live `shadow_trades` table specifically until now.

**New finding, not previously documented anywhere:** `template_source` **is** a real column on `shadow_trades`, and the notebook never mentions it at all. This is a deliberate schema design beyond the notebook — almost certainly because whether a shadow trade's benchmark came from a real historical template match versus a fallback matters specifically for evaluating automation readiness, which is this table's entire purpose. `ShadowTradeService` now populates it from `RecommendationDiagnostics.templateSource` (Section 3.9).

**`confidence_label`** (Section 12 item 2, already known to lack a notebook formula) is populated as the **literal value `null`**, typed as a literal rather than `'LOW'|'MEDIUM'|'HIGH'|null`, mirroring the same discipline already used for `visibleToAnalyst: false` — any future attempt to populate it with a real formula becomes a deliberate, visible type change requiring its own architecture amendment, not a silent value change that could slip past review.

### Scope confirmed: creation only

This step covers shadow trade *creation* — one `shadow_trades` row plus a paired initial `shadow_trade_outcomes` row at `NOT_TRIGGERED`. Outcome *evolution* (Section 12 item 3, the logic that would move a shadow trade's status to `TRIGGERED`/`TARGET_HIT`/`STOP_HIT`/`EXPIRY` as the market actually moves) remains genuinely unaddressed — it has zero notebook precedent and needs its own design pass, exactly as already flagged. Scheduling it as Roadmap Step 7a (since V1.1/Amendment 8) does not substitute for that design work.

No other section is affected. All prior amendments remain in force exactly as stated in their respective changelogs.

---

## Changelog: V1.5 → V1.6 (preserved for history)

### New service: `reassessRecommendationBatch()` (`recommendationReassessmentService.ts`)

Per Section 5.1's two-trigger-path design (already documented since V1.1, only now actually built): given a batch of currently-active recommendations (each carrying its stored `priceAtGeneration`/`zoneAtGeneration`) and a map of **freshly, independently fetched** `MarketStateOutput` per market, re-runs `assessCondition()` for each — this time comparing against real, current data, not the same snapshot used at generation. This is where real `STALE_PRICE`/`ZONE_CHANGED`/`DO_NOT_USE_RECALCULATE` detection actually happens in practice.

**New output field: `requiresRegeneration`.** True if and only if `recommendationValidityStatus === 'DO_NOT_USE_RECALCULATE'` — deliberately narrower than the existing `requiresRefresh` flag (which is also true for `STALE_PRICE`/`ZONE_CHANGED`, states that warrant caution, not outright regeneration). Per Section 5.1's regeneration workflow, only `DO_NOT_USE_RECALCULATE` should trigger `RecommendationService` being called again to produce version N+1.

**A market with no fresh state in the input map is treated as missing/incomplete data**, falling through to `DO_NOT_USE_RECALCULATE` via `assessCondition`'s own existing guard (passing `currentPrice: null, atr14: null` rather than inventing a new special case) — reusing logic that already exists and is already tested, not duplicating it.

**Remains a pure function**, consistent with every other service in this engine — no I/O, no database access. The actual orchestration (fetching the batch of active recommendations and fresh market states on a schedule, calling this function, persisting the updated validity fields, and triggering `RecommendationService` for anything flagged `requiresRegeneration`) belongs to Phase 1.3's `EngineOrchestrationService`, which is SQL/engine-run-driven and has not been built as TypeScript in this package — that integration work remains ahead, not part of this amendment.
...
### 1. Resolves Section 12 item 1a — analyst-facing display precision is now market-aware, not hardcoded

`markets.display_precision` (new column, `031_display_precision_and_condition_fields.sql`) is the source of truth, with a documented last-resort fallback (4dp, FX-biased) only when null. `formatMarketPrice(price, displayPrecision)` is the new formatting primitive; `formatGuidanceRange()` now requires `displayPrecision` as an explicit input rather than defaulting silently. V1 backfill defaults by `asset_class`, with a symbol-pattern override for JPY-quoted FX pairs (3dp vs. 4dp for other FX majors): FX majors 4dp, JPY pairs 3dp, commodities/indices/crypto/equities 2dp (commodities not differentiated further — Gold/Oil/Copper/Natural Gas/Brent all 2dp in V1; crypto not differentiated per-symbol in V1). This rule is presentation-only — confirmed by construction, not just by convention: `formatMarketPrice` is called nowhere in the calculation pipeline, only at the final point of producing `riskRange`/`targetRange` text, so it cannot affect `entryMid`/`stop`/`target`/`expectedR`/`triggerProbability`/any hash.

### 2. Resolves Section 12 item 1b — `volatilityWarning`/`atrMoveSinceGeneration` are now real columns

`recommendation_versions.volatility_warning` (text) and `recommendation_versions.atr_move_since_generation` (numeric), same migration. Per product clarification: these explain *why* a recommendation became stale, support manager review and post-trade context, and must be persisted at the time of assessment rather than recomputed later from market data that has since moved on. `RecommendationService` already produced these values (Section 3.9's `RecommendationVersionOutput`) — this closes the schema gap, no service-logic change was needed.

### 3. `parameter_snapshot_hash` backfill for pre-existing rows — explicitly NOT attempted in SQL

Recomputing `stableHash(parameter_snapshot)` for any pre-existing row requires the exact same hashing algorithm as `stableHash.ts` (verified byte-for-byte against the notebook's Python implementation in Step 1 — a deep-sorted, specifically-separated JSON serialization, not a generic hash). Reimplementing that precisely in `plpgsql` risks exactly the subtle separator/serialization mismatches already found and fixed once in the JS/Python comparison. Given `recommendation_versions` has not yet been written to by real application code (Step 5's services have only run in tests so far), this is very likely moot in practice. **Decision:** if real rows ever exist with `parameter_snapshot` set but `parameter_snapshot_hash` null before the application itself computes it, the correct tool is a one-off Node/TS backfill script reusing the verified `stableHash()` function — not a SQL rewrite of the algorithm. Not built now, since there's nothing to backfill yet; flagged here so it isn't forgotten if that changes.

No other section is affected. V1.4's schema restructuring and all earlier amendments remain in force exactly as stated in their respective changelogs.

---

## Changelog: V1.3 → V1.4 (preserved for history)

This amendment was triggered by checking `RecommendationService`'s output against the real live schema (`information_schema.columns` queried directly for `recommendation_versions`, `opportunities`, `coverage_allocation`) rather than assuming the notebook's flat output shape would map onto it — and finding it didn't, in a fundamental way, not a missing-field way.

### 1. `RecommendationService` now returns three structurally separate things, not one flat object

| Output | Maps to | Purpose |
|---|---|---|
| `OpportunityOutput` | `opportunities` table | One row per market/date/session — `market`, `session`, `date`, `direction`, `currentZone`, `preferredEntryZone`, `analystAction`, `expectedR`, `triggerProbability`, `assignedAnalystId` (a *preference*), `opportunityLifecycleStatus` |
| `RecommendationVersionOutput` | `recommendation_versions` table | One row per version of the entry/stop/target geometry — `entryRangeLow/High`, `riskRange`/`targetRange` (**text**, not raw numbers), `recommendationValidityStatus`, `requiresRefresh`, `zoneAtGeneration`, `priceAtGeneration`, `eventRiskStatus`, `regimeTags`, `parameterSnapshot(Hash)` |
| `HiddenExecutionLevels` | Not a table — input to the future `ShadowTradeService` (Step 7) only | Precise numeric `entryMid`/`stop`/`target`/`rr` — never analyst-facing, never persisted to `recommendation_versions` |

`expectedR` and `triggerProbability` live on the **opportunity**, not the recommendation version — a real structural finding from the schema, not an assumption: the opportunity is "which trade idea is this," updated in place; the recommendation version is "what does the entry/stop/target geometry currently look like," versioned as price moves and re-assessed. These have different lifecycles and belong on different rows.

### 2. Analyst-facing guidance is fundamentally different from hidden shadow-trade execution — this distinction has no notebook equivalent at all

Product clarification (not research-derived): the notebook only ever computes one stop/target, used identically for both internal benchmarking and (implicitly, if it were analyst-facing at all) display. Production needs two different things from the same anchor:

- **Hidden shadow trade** (`HiddenExecutionLevels`): exact numeric levels, used as real, precise execution for the internal benchmark that measures whether the platform's own logic could produce a "clean, mistake-free" recommendation. Never shown to an analyst.
- **Analyst-facing guidance** (`riskRange`/`targetRange`, text): a deliberately imprecise *area worth considering*, not an executable order. Coaching philosophy, not trading instruction — "Buy exactly here" is explicitly the wrong framing; "this is the area worth considering today" is the right one.

For V1, the analyst-facing range is NOT analyst-profile-adjusted — it is a deterministic, symmetric tolerance band (`anchorPrice ± 0.125 × ATR`) around the same ATR-anchored level the hidden shadow trade uses, formatted as text (e.g. `"1.0825–1.0835"`) via a new `formatGuidanceRange()` helper (`guidanceRangeFormatter.ts`). An analyst-profile-adjusted (asymmetric, edge-aware) range is an explicitly deferred, research-validation-gated future enhancement — the `direction`/`type` parameters in `formatGuidanceRange()`'s signature are reserved for this, not currently used in the V1 formula.

**Open question, not resolved by this amendment:** decimal precision for the guidance text defaults to 4 (FX convention) but is exposed as an explicit parameter, since the engine also covers indices/commodities with very different natural magnitudes (an index around 4500.25, needing 2dp, not 4). No per-asset-class precision rule exists yet — flagged in Section 12, not assumed away.

### 3. Template/profile diagnostic fields are NOT duplicated onto either persisted output

Per product clarification: `template_quality`, `profile_quality`, `templateAvgR`, etc. are not persisted as columns on `recommendation_versions` in V1 — a reader joins back to `template_profiles`/`analyst_profiles` instead, avoiding duplication. They remain available in a new `RecommendationDiagnostics` return value, explicitly marked as a debugging/testing aid, never a persisted shape. `eligibleAnalysts` (the full list) similarly has no column home on either `opportunities` or `recommendation_versions` — it lives only in `coverage_allocation` (`AllocationService`'s job, Step 8) and in `diagnostics` for now.

### 4. `assignedAnalyst`/`eligibleAnalysts` were removed from the recommendation-version-level output entirely

`AnalystProfileService`'s pick (`select_best_analyst`) is a **preference**, persisted on the *opportunity* as `assignedAnalystId`. The **final** allocation decision is `AllocationService`'s job (`coverage_allocation`, Step 8, not yet built) — a separate table, a separate score (`allocation_score`), and a separate set of eligible analysts (`coverage_allocation.eligible_analysts`). These were conflated into one field in V1.1–V1.3's documented contract; they are now correctly separated.

### 5. `parameter_snapshot_hash` is now a real column, not nested-only inside `parameter_snapshot`

Per product clarification: used frequently for equivalence checks (`ValidationService`), should not require re-hashing the full jsonb every time, and supports indexing/filtering. Migration: `030_recommendation_versions_parameter_snapshot_hash.sql`. The jsonb `parameter_snapshot` still carries the complete parameters; the hash column is a fast, precomputed lookup key, not a second source of truth.

### 6. `opportunities.version` and `coverage_allocation.lock_version` are confirmed as genuinely different concepts

`opportunities.version` is a **domain** version (opportunity regeneration, audit, "has this opportunity been revised"). `coverage_allocation.lock_version` is **purely optimistic locking** (concurrent-write protection, matching Phase 1.3's established pattern elsewhere in the schema). Do not conflate or reuse one for the other — confirmed explicitly, not inferred.

No other section is affected. V1.2's corrections and V1.3's NaN-handling decisions remain in force exactly as stated there.

---

## Changelog: V1.2 → V1.3 (preserved for history)
...
No other section was affected by this amendment. V1.2's corrections (Sections 3.4, 3.5, 12 item 1) remained in force exactly as stated there.
...
This is the approved, frozen baseline (V1.11) — the seven resolutions in Section 1, the V1.1-V1.11 amendments (see Changelogs above), and the Section 3.9 schema restructuring have already been reviewed and approved. Section 1 is still the most important section to read first, since every other section depends on its resolutions, but it is no longer a set of open recommendations awaiting sign-off — it is the contract. Any future disagreement with a resolution here is a proposed amendment against this baseline (a new versioned document, V1.12 and onward), not an in-place edit.
...
Notebook's `allocate_coverage` scores each eligible analyst as `expected_r + (0.2 if already-assigned-by-profile else 0) − workload×0.1`, then assigns the single highest scorer, updating an in-memory workload counter as it goes.

**[CORRECTED in V1.9 — see Changelog]:** V1.1's original resolution described dedicated columns for `market_fit_score`, `regime_fit_score`, `workload_score`, `availability_score`, and `final_score` on `coverage_allocation`. Confirmed against the live schema before building Step 8: **only `allocation_score` exists**. The notebook only ever computes one combined score — the five-column description was an imagined future extension, not a current schema reality.

**Resolution (corrected):** Implement exactly the notebook's formula for v1 — one combined `allocation_score` = `expected_r + (0.2 if preferred_analyst else 0) − workload×0.1`. `analyst_availability` is incorporated as a hard eligibility filter (an unavailable analyst does not appear in `eligible_analysts` at all), which is a justified, conservative extension beyond the notebook's static `ACTIVE_ANALYSTS` table — ignoring a live availability table that already exists would be actively worse than using it.

### 1.8 — Analyst-facing guidance vs. hidden shadow-trade execution [new in V1.4, see Changelog]

A real product distinction with no notebook equivalent at all. The notebook computes one stop/target, used for everything. Production needs two:

- **Hidden shadow trade**: exact numeric `entryMid`/`stop`/`target`/`rr` (`HiddenExecutionLevels`), used as real precise execution for the internal benchmark (`ShadowTradeService`, Context 4, Step 7). Never analyst-facing.
- **Analyst-facing guidance**: `riskRange`/`targetRange`, deterministic TEXT, an *area worth considering*, never an executable order. Coaching philosophy applies directly here: "this is the area worth considering today," never "buy exactly here."

**Resolution (V1 formula):** `formatGuidanceRange()` (`guidanceRangeFormatter.ts`) produces a symmetric tolerance band, `anchorPrice ± 0.125 × ATR`, anchored on the same ATR-derived stop/target the hidden shadow trade uses. Explicitly NOT analyst-profile-adjusted in V1 — an asymmetric, edge-aware range is a deferred, research-validation-gated future enhancement (the formatter's `direction`/`type` parameters are reserved for this, unused in the V1 formula). **Display precision is market-aware [V1.5]:** `formatMarketPrice(price, displayPrecision)` sources precision from `markets.display_precision` (FX majors 4dp, JPY pairs 3dp, commodities/indices/crypto/equities 2dp in the V1 backfill), with a documented 4dp fallback only when a market's value is null — never hardcoded universally. This is presentation-only by construction: `formatMarketPrice` is called nowhere in the calculation pipeline, only at the final point of producing display text, so it cannot affect `entryMid`/`stop`/`target`/`expectedR`/`triggerProbability`/any validation hash.
...
**[REPLACED in V1.4 — see Changelog. The contract below was verified directly against the live `opportunities`/`recommendation_versions` schema and an explicit product clarification distinguishing analyst-facing guidance from hidden shadow-trade execution. It replaces V1.1-V1.3's single flat-object contract entirely, not just a few fields within it.]**

```typescript
interface BuildRecommendationInput {
  recommendationVersionId: string; // caller-generated, kept as an explicit input for purity/determinism (Section 7)
  generatedAt: string;             // ISO timestamp, caller-supplied for the same reason
  market: string; session: SessionType;
  marketState: MarketStateOutput; marketRegime: MarketRegimeOutput | null;
  eventRisks: MarketEventRiskOutput[]; // for this market; highest riskScore wins if multiple
  trades: RecommendationInputTrade[]; activeAnalysts: ActiveAnalyst[];
  ...
  parameterSnapshot: Record<string, unknown>; parameterSnapshotHash: string;
}

// Maps to the `opportunities` table.
interface OpportunityOutput {
  opportunityId: string; market: string; session: SessionType; date: string;
  direction: 'BUY' | 'SELL'; currentZone: AtrZone | null; preferredEntryZone: AtrZone;
  analystAction: 'ENTER_NOW' | 'WAIT_FOR_PREFERRED_ZONE';
  expectedR: number; triggerProbability: number;
  assignedAnalystId: string | null; // a PREFERENCE -- AllocationService's coverage_allocation may differ, see Section 3.5
  opportunityLifecycleStatus: 'DRAFT'|'GENERATED'|'ASSIGNED'|'SHOWN'|'ACTIVE'|'CLOSED'|'CANCELLED'; // set to 'GENERATED' here
}

// Maps to the `recommendation_versions` table -- analyst-facing fields only.
interface RecommendationVersionOutput {
  recommendationVersionId: string; opportunityId: string; versionNumber: number;
  entryRangeLow: number; entryRangeHigh: number;
  riskRange: string; targetRange: string; // deterministic guidance TEXT, never an executable level -- see Section 1.8 (new)
  recommendationValidityStatus: RecommendationValidityStatus; requiresRefresh: boolean;
  zoneAtGeneration: AtrZone | null; priceAtGeneration: number;
  eventRiskStatus: string; regimeTags: string[];
  parameterSnapshot: Record<string, unknown>; parameterSnapshotHash: string;
  volatilityWarning: string; atrMoveSinceGeneration: number | null; // real columns as of V1.5, see Changelog
}

// NOT a table. Input to the future ShadowTradeService (Step 7) ONLY.
interface HiddenExecutionLevels { entryMid: number; stop: number; target: number; rr: number; }

// NOT a persisted shape. Debugging/testing aid only.
interface RecommendationDiagnostics {
  templateSource: 'historical_template'|'fallback'; templateAvgR: number; templateWinRate: number|null;
  templateTrades: number; templateQuality: 'HIGH'|'MEDIUM'|'LOW';
  profileSource: 'exact_profile'|'market_profile'|'fallback'; profileAvgR: number; profileWinRate: number|null;
  profileTrades: number; profileQuality: 'HIGH'|'MEDIUM'|'LOW';
  eligibleAnalysts: string[]; eventWarning: string;
}

interface BuildRecommendationOutput {
  opportunity: OpportunityOutput; recommendationVersion: RecommendationVersionOutput;
  hiddenExecutionLevels: HiddenExecutionLevels; diagnostics: RecommendationDiagnostics;
}
```
...
**Invariant — the three outputs are structurally separate, not three views of one object.** `hiddenExecutionLevels` must never be merged into `recommendationVersion` — this mirrors the same type-level discipline as `ShadowTradeService`'s `visibleToAnalyst: false` literal (Section 3.11): a future accidental analyst-facing read of "the recommendation" must not be able to pick up precise execution levels by construction, not merely by convention.

**Invariant — `expectedR`/`triggerProbability` belong on the opportunity, not the version.** They describe "which trade idea is this," not "what does the current entry/stop/target geometry look like" — the opportunity is updated in place as the underlying trade idea's quality is reassessed; the recommendation version is what gets re-versioned as price moves. Do not move these fields onto `RecommendationVersionOutput` even though earlier drafts of this document had them there.

**Invariant — `assignedAnalystId` is a preference, never a final decision.** Per Section 3.5, `AllocationService`'s `coverage_allocation.assigned_analyst_id` (Step 8) is authoritative and can differ. `eligibleAnalysts` (the full list) has no column home on `opportunities` at all — it exists only in `coverage_allocation.eligible_analysts` and, pending that, in `diagnostics`.
...
**Maps to:** notebook cell 11, `build_shadow_trades`. **[CORRECTED in V1.7 — see Changelog. The contract below replaces V1.1-V1.6's text, which was not yet checked against the live `shadow_trades` schema.]** Per Section 1.6, outcome evolution beyond creation is **not** covered by validated research and remains an open implementation gap (Section 12) — this service's contract covers creation only.

```typescript
type TemplateSource = 'historical_template' | 'fallback' | 'exact_profile' | 'market_profile' | 'unknown'; // widened in V1.8 -- see Changelog

interface CreateShadowTradeInput {
  shadowTradeId: string; shadowOutcomeId: string; createdAt: string; // all caller-supplied -- pure function
  recommendationVersionId: string; opportunityId: string;
  entry: number; stop: number; target: number; rr: number; // from HiddenExecutionLevels, never the analyst-facing text range
...
interface ShadowTradeOutput {
  ...
  confidenceLabel: null; // literal type -- see invariant below
  visibleToAnalyst: false; // literal type, enforced at compile time
  createdAt: string;
}

// Maps to the `shadow_trade_outcomes` table -- initial row only.
interface ShadowTradeOutcomeOutput {
  shadowOutcomeId: string; shadowTradeId: string;
  tradeOutcomeStatus: 'NOT_TRIGGERED'; // literal -- this service never produces any other status
  resultR: null; outcomeTimestamp: null; createdAt: string;
}
```

**Invariant — `market`/`session`/`direction`/`triggerProbability`/`expectedR`/`parameterSnapshotHash`/`shadow_status` are NOT part of this contract**, even though the notebook copies all of them onto the shadow trade record. Confirmed against the real `shadow_trades` columns directly — none of these exist there. They are recoverable via the `recommendationVersionId`/`opportunityId` FKs and a join, never duplicated.

**Invariant — `templateSource` is real and required**, despite the notebook never mentioning it. A genuine schema addition beyond the notebook, almost certainly because whether a shadow trade's benchmark came from a real historical template match or a fallback matters specifically for evaluating automation readiness — this table's entire purpose.
...
**Invariant — `confidenceLabel` is the literal `null`, not `'LOW'|'MEDIUM'|'HIGH'|null`.** The notebook never computes a confidence label for shadow trades, and no research-validated formula exists (Section 12 item 2, still open). Typing this as a literal — the same discipline as `visibleToAnalyst` — means a future attempt to populate it with a real value is a deliberate, visible type change requiring its own amendment, not a silent value change.
...
| 6 | ✅ **DONE (V1.6).** `RecommendationLifecycleService`'s periodic re-check (`reassessRecommendationBatch()`, Section 5.1), with the implemented/reserved state guard from Amendment 7. Pure-function half complete and tested (104/104). **Still pending:** the actual scheduled orchestration that calls this function on a cadence, fetches fresh market states, persists results, and triggers regeneration — that's Phase 1.3's `EngineOrchestrationService` integration, not yet built in this package. | Step 5 | None — reuses existing `recommendation_versions` versioning |
| 7 | ✅ **DONE (V1.7).** `ShadowTradeService` creation logic, corrected against the real schema (104→111/111 tests). `template_source` populated; `confidence_label` set to literal `null` pending a research-validated formula. | Step 5 | New `insert_shadow_trade` SQL function -- not yet written, still pending |
| 7a | **`ShadowOutcomeLifecycleService`** [explicitly named, V1.8] (Amendment 8 — promoted from open ambiguity to a defined work item, now with a real name and a stated boundary): tracks a shadow trade's evolution from `NOT_TRIGGERED` through `TRIGGERED`/`TARGET_HIT`/`STOP_HIT`/`EXPIRY`/`CANCELLED`/`AMBIGUOUS` (the full live `shadow_trade_outcomes.trade_outcome_status` enum) as the market actually moves. **Must be designed and built as a service entirely separate from `ShadowTradeService`** — `ShadowTradeService` creates the immutable benchmark setup and must never be extended to also monitor or resolve outcomes. Inputs: the shadow trade's immutable entry/stop/target (from Step 7), an expiry rule, intraday market data, session rules. Does not need to complete before Step 5/9, per Amendment 8, but is no longer deferrable to an undefined future phase, and naming it does not substitute for the still-needed design pass (Section 12 item 3). | Step 7, `MarketStateService` (Step 1) for fresh price checks | New `update_shadow_trade_outcome` SQL function, plus a periodic engine step (`shadow_outcome_check`) alongside the existing validity-check cadence |
| 8 | ✅ **DONE (V1.9).** `AllocationService` — stateful sequential algorithm, single `allocation_score` column (V1.1's five-column claim corrected), `reason_summary` persisted as a literal fixed string. 121/121 tests passing. | Steps 5, 6 | None — `coverage_allocation` already has all needed columns, confirmed before building |
| 9 | ✅ **DONE (V1.10).** `ValidationService` + `engine_validation_runs` table (migration 032, including `resolution_type` per Amendment 4). 135/135 tests passing. Severity classification for `recommendationValidityStatus` corrected against the architecture document's exact definition (not all mismatches CRITICAL -- only those involving `DO_NOT_USE_RECALCULATE`). **Note:** the fixture-freezing process (Section 10.3) and CI gate integration are still ahead -- this step delivers the pure comparison logic only, consistent with the rest of this engine. | Steps 1-8 all functional | `032_engine_validation_runs.sql` |
...
1a. ~~Decimal precision for `formatGuidanceRange()`'s analyst-facing text~~ — **RESOLVED in V1.5.** `markets.display_precision` is now the source of truth, with a documented fallback only when null. See Section 1.8 / Changelog V1.4→V1.5 item 1.
1b. ~~`volatilityWarning`/`atrMoveSinceGeneration` have no column on `recommendation_versions`~~ — **RESOLVED in V1.5.** Both are now real columns (`031_display_precision_and_condition_fields.sql`). See Changelog V1.4→V1.5 item 2.
2. **`shadow_trades.confidence_label` derivation** (Section 3.11): no notebook formula exists. Needs either a research-validated formula or an explicit decision to leave it `NULL`/a fixed default until one exists. Per V1.8's clarification, a future formula should draw on template sample size, template average R, trigger probability sample size, profile sample size, event risk, regime stability, and shadow-vs-actual performance — recorded as a pointer for that future work, not a formula to implement now.
3. **Shadow trade outcome evolution now has a name (`ShadowOutcomeLifecycleService`, V1.8) but is still entirely undesigned.** Naming and bounding it (Section 3.11's V1.8 update: tracks `NOT_TRIGGERED`→`TRIGGERED`→`TARGET_HIT`/`STOP_HIT`/`EXPIRY`/`CANCELLED`/`AMBIGUOUS`, designed separately from `ShadowTradeService`) does not answer the actual design question — the exact re-evaluation logic (what counts as hitting target vs. stop, intraday vs. daily price checks, how `EXPIRY` is determined) still has zero validated research behind it and needs its own design pass before any code is written.
3a. **The path from `RecommendationService`'s diagnostics to `ShadowTradeService`'s `templateSource` input is not yet wired anywhere** (Section 3.11, V1.8): `ShadowTradeService` deliberately does not decide which of its five `templateSource` values applies for a given recommendation — that decision belongs to whatever calls it, which does not exist yet (it's part of the still-unbuilt `EngineOrchestrationService` integration). Whether that caller should prefer `TemplateService`'s classification or `AnalystProfileService`'s when both are available, or some other rule, is a real open question, not yet decided.
...
*End of APIP Intelligence Engine Architecture V1.11 — frozen baseline. This document, not the notebook directly, is the contract implementation must satisfy. Future architectural changes are proposed as versioned amendments against this baseline (V1.12, V1.13, ...), never as direct edits to this file.*
