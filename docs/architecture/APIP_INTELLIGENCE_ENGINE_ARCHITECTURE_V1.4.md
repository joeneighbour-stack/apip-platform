# APIP Intelligence Engine Architecture V1.4 — FROZEN BASELINE
...
**Supersedes:** V1.3. This is the most structurally consequential amendment so far: `RecommendationService`'s output shape, as documented in every prior version, was wrong — not in a formula or threshold, but in its basic structure. Verified directly against the live `opportunities`/`recommendation_versions`/`coverage_allocation` schema (not assumed) and against an explicit product clarification distinguishing analyst-facing guidance from hidden shadow-trade execution — a real distinction with no notebook equivalent at all.
...
This is the approved, frozen baseline (V1.4) — the seven resolutions in Section 1, the V1.1-V1.4 amendments (see Changelogs above), and the Section 3.9 schema restructuring have already been reviewed and approved. Section 1 is still the most important section to read first, since every other section depends on its resolutions, but it is no longer a set of open recommendations awaiting sign-off — it is the contract. Any future disagreement with a resolution here is a proposed amendment against this baseline (a new versioned document, V1.5 and onward), not an in-place edit.
...
**Resolution (V1 formula):** `formatGuidanceRange()` (`guidanceRangeFormatter.ts`) produces a symmetric tolerance band, `anchorPrice ± 0.125 × ATR`, anchored on the same ATR-derived stop/target the hidden shadow trade uses. Explicitly NOT analyst-profile-adjusted in V1 — an asymmetric, edge-aware range is a deferred, research-validation-gated future enhancement (the formatter's `direction`/`type` parameters are reserved for this, unused in the V1 formula). Decimal precision defaults to 4 (FX convention) but is an explicit parameter, not hardcoded — no per-asset-class precision rule exists yet (Section 12 item 1a).
...
interface RecommendationVersionOutput {
  ...
  parameterSnapshot: Record<string, unknown>; parameterSnapshotHash: string;
  volatilityWarning: string; atrMoveSinceGeneration: number | null; // no fixed column yet -- see Section 12
}
...
1a. **Decimal precision for `formatGuidanceRange()`'s analyst-facing text** (Section 1.8, new in V1.4): defaults to 4dp (FX convention) but no per-asset-class precision rule exists — an index around 4500.25 would display with a misleading number of decimal places under the default. Needs a real rule (e.g. derived from the market's asset class, or from typical price magnitude) before this is correct across the full market universe, not just FX pairs.
1b. **`volatilityWarning`/`atrMoveSinceGeneration` have no column on `recommendation_versions`** (Section 3.9, new in V1.4): confirmed against the live schema, not assumed missing. Both are cheap, useful computed values currently returned by `RecommendationVersionOutput` with no persistence path — needs a decision on whether to add columns, fold them into an existing jsonb field, or accept they're simply not persisted (recomputable on demand from `recommendationValidityStatus` and the stored `priceAtGeneration`/`zoneAtGeneration`, at the cost of needing a fresh market state fetch to regenerate the human-readable warning text).
...
*End of APIP Intelligence Engine Architecture V1.4 — frozen baseline. This document, not the notebook directly, is the contract implementation must satisfy. Future architectural changes are proposed as versioned amendments against this baseline (V1.5, V1.6, ...), never as direct edits to this file.*
