# APIP Intelligence Engine Architecture V1.1 — FROZEN BASELINE
...
**Supersedes:** V1.0 (draft), approved subject to 8 amendments, all incorporated below.
...
**Inputs to this document:** `APIP_NEXTJS_PLATFORM_SPEC_V1_6.xlsx` (defines WHERE — schema, lifecycle, governance), `APIP_RESEARCH_ENGINE_V1_0.ipynb` (defines HOW — the validated behavioural reference, and now a first-class, permanently-comparable citizen of this architecture per Amendment 4), and the as-built Phase 1.1–1.4 schema (44 tables, already live in Supabase staging, `migrations/001`–`027`).

## Changelog: V1.0 → V1.1

| # | Amendment | Section(s) changed |
|---|---|---|
| 1 | `OpportunityService` documented as a deliberate extension point, not a permanent pass-through | 1.4 |
| 2 | Historical Entry Zone Reconstruction feasibility assessment | 1.5 |
| 3 | `ValidationService` extended with a Behavioural Difference Engine | 10.5 (new) |
| 4 | Research notebook elevated to first-class, permanently-compared architectural citizen | 10.6 (new) |
| 5 | Service boundaries reviewed for future event-driven evolution | 5.2 (new) |
| 6 | Domain language — produced as a companion document, `APIP_DOMAIN_LANGUAGE_V1.md`, not inline here | New section 13 references it |
| 7 | Implemented-vs-reserved enum states given an explicit code-level convention, not just documentation | 1.1 |
| 8 | Shadow Outcome Lifecycle promoted from "open ambiguity" to a defined roadmap work item | 11 (Step 7 renamed/expanded), 12 |
...
interface SelectBestTemplateInput { market: string; templates: TemplateProfile[]; minTemplateTrades: number; }
...
- Selection criteria: `trades >= minTemplateTrades` (20), sorted by `(avgR desc, trades desc, winRate desc)`, take the first. If no row clears the trade-count floor, return the exact fallback (`templateSource: 'fallback'`, `direction: 'BUY'`, `preferredEntryZone: ZONE_1`, `templateAvgR: 0`) — this fallback is itself part of the validated behaviour, not a placeholder to improve on.
- Per Section 1.5, `entryZone` will be `NULL` for the large majority of backfill rows. `templateQuality` must reflect this honestly. **Open question for implementation** — flagged in Section 12.

### 3.5 AnalystProfileService

**Maps to:** notebook cell 8, `build_analyst_profiles`, `select_best_analyst`. Same shape and same invariants as 3.4, with `analyst` added to the grouping key, and eligibility filtered by `active_analysts[session] == true` (live equivalent: `analysts.sessions` array, joined against `analyst_availability` for the specific date per Section 1.7's recommendation).
...
1. **Zone-conditioning for `NULL`-zone historical trades** (Section 3.4): exclude entirely from zone-conditioned groupings, or pool into a distinct "zone unknown" bucket? The notebook's sample data never exercises this case, so there's no validated answer to point to.
